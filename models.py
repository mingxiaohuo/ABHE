...
def disjoint_augment_image_pair(train_inputs, min_val=-1, max_val=1):
    ...
    random_brightness = tf.random.uniform([], 0.7, 1.3)
    img1_aug = img1 * random_brightness
    random_brightness = tf.random.uniform([], 0.7, 1.3)
    img2_aug = img2 * random_brightness
    
    # Randomly shift color
    random_colors = tf.random.uniform([3], 0.7, 1.3)
    white = tf.ones([tf.shape(input=img1)[0], tf.shape(input=img1)[1], tf.shape(input=img1)[2]])
